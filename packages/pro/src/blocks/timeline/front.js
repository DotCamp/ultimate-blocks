...
class UltimateBlocksTimeline {
  constructor(wrapper) {
    this.timelineWrapper = wrapper;
    this.timelineTree =
      this.timelineWrapper?.querySelector(".ub-timeline-tree");
    this.timelineTreeProgress = this.timelineTree?.querySelector(
      ".ub-timeline-tree-progress"
    );
    this.timelineItems =
      this.timelineWrapper?.querySelectorAll(".ub-timeline-item");

    this.init();
  }
  init() {
    if (this.timelineWrapper.classList.contains("ub-timeline-vertical")) {
  ...
  isElementAboveVerticalCenter(element) {
    ...
    return elementVerticalCenter <= viewportVerticalCenter + OFFSET;
  }
  timelineProgress() {
    const timelineTreeHeight = this.timelineTree?.clientHeight;
    const windowDistance = window.scrollY;
    const windowHeight = window.innerHeight / 2;
    const timelineWrapperOffset = this.timelineWrapper.offsetTop;
    const progress = windowDistance - timelineWrapperOffset + windowHeight;
    if (progress <= timelineTreeHeight) {
      this.timelineTreeProgress.style.height = progress + 1 + "px";
    }
    if (progress > timelineTreeHeight) {
      this.timelineTreeProgress.style.height = timelineTreeHeight + "px";
    }
    if (progress < 0) {
      this.timelineTreeProgress.style.height = 0 + "px";
    }
    this.timelineItems.forEach((item, index) => {
      const connector = item?.querySelector(".ub-timeline-item-connector");
      if (this.isElementAboveVerticalCenter(connector)) {
        connector.classList.add("ub-timeline-active-item-connector");
      } else {
        connector.classList.remove("ub-timeline-active-item-connector");
      }
    });
  }
}
document.addEventListener("DOMContentLoaded", function () {
  const timelineWrapper = document?.querySelectorAll(
    ".ub-timeline-wrapper.ub-timeline-show-progress"
  );
  timelineWrapper.forEach((wrapper) => new UltimateBlocksTimeline(wrapper));
  const swiperWrappers = document.querySelectorAll(
    ".swiper.ub-timeline-horizontal"
  );
  swiperWrappers.forEach((swiperWrapper) => {
    const itemsPerView = swiperWrapper.getAttribute("data-items_per_view");
    const showArrow = swiperWrapper.getAttribute("data-show_arrow") === "true";
    const enableDragging =
      swiperWrapper.getAttribute("data-enable_dragging") === "true";
    const config = {
    ...
    const swiper = new Swiper(swiperWrapper, config);
  });
});
