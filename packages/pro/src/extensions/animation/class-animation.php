...
class Ultimate_Blocks_Animation  {
    ...
    public function ub_render_animation($block_content, $block){
        ...
        if ($is_animation_selected) {
            $classes = array();
            $styles  = array();
            $is_animation_repeat = isset($attributes['animationRepeat']) ? $attributes['animationRepeat'] : "repeat";
            array_push($classes, "animated");
            if(isset($attributes['selectedAnimation'])){
                array_push($classes, $attributes['selectedAnimation']);
            }
            if($is_animation_repeat){
                array_push($classes, $is_animation_repeat);
            } else {
                array_push($classes, 'repeat');
            }
            if ($is_animation_selected && isset($attributes['animationRepeatCount']) && $is_animation_repeat === "repeat") {
                $styles["--animate-repeat"] = $attributes['animationRepeatCount'];
            }

            if (isset($attributes['animationDuration']) && is_numeric($attributes['animationDuration'])) {
                $styles["--animate-duration"] = $attributes['animationDuration'] . "s";
            }
            if (isset($attributes['animationDelay']) && is_numeric($attributes['animationDelay'])) {
                $styles["--animate-delay"] = $attributes['animationDelay'] . "s";
            }
            ...
            if (preg_match('/style="([^"]*)"/i', $first_tag, $matches)) {
                ...
                $style_attribute = ' style="' . $existing_styles . ';';
			...
			$first_tag_replaced = str_replace('class="', 'class="' .implode(' ', $classes). ' ', $first_tag_replaced);
