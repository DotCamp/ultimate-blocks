...
import SavedStylesListing from '@Components/SavedStyles/SavedStylesListing';
...
} from '@Stores/savedStyles/selectors';
import { withHookManager } from '@Managers/HookManager';
import SavedStylesAdvancedControls from '@Components/SavedStyles/SavedStylesAdvancedControls';
 ...
 * @param {Function}      props.setSelectedItemId             set id for selected style, will be supplied via HOC
 * @param {Function}      props.setCurrentBlockTypeOverride   set current block type override, will be supplied via HOC
 * @param {boolean}       [props.visibility=true]             whether to show controls panel or not, supplying false will only hide controls panel, but will not affect inner workings of the component
...
function SavedStylesInspectorPanel({
	...
	useEffect(() => {
		...
		if (isSubcomponentMarkedForDefaultStyle()) {
			attributes.applyDefaultStyle = false;
	...
	return (
		<PanelBody
			className={visibility ? '' : 'ub-pro-saved-styles-inspector-hidden'}
			title={__('Styles', 'ultimate-blocks-pro')}
		>
