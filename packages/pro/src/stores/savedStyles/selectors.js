  ...
  getComponentTypeFromStyleId(state, styleId) {
    ...
    if (state.saved.styles) {
      componentType = Object.keys(state.saved.styles)
        .filter((key) =>
          Object.prototype.hasOwnProperty.call(state.saved.styles, key)
        )
        .reduce((carry, cKey) => {
          const cStyles = Object.keys(state.saved.styles[cKey]).filter((sId) =>
            Object.prototype.hasOwnProperty.call(state.saved.styles[cKey], sId)
          );
