...
class Static_Styles_Manager {
	 ...
	 * @var string
	 */
	private $static_data_path = 'inc/admin/data/savedStyles.ubdata';
	...
	protected function init_process() {
		if ( current_user_can( 'administrator' ) ) {
			add_filter( 'ub-pro/filter/savedStylesFrontendData', [ $this, 'add_frontend_data' ] );
			add_action( 'rest_api_init', [ $this, 'register_rest_endpoints' ] );
		}

		add_filter( 'ub-pro/filter/savedStylesFrontendData', [ $this, 'add_static_styles' ] );
		add_filter( 'ub-pro/filter/savedStylesSaveData', [ $this, 'filter_static_styles' ] );
	}
	...
	public function filter_static_styles( $saved_styles ) {
		...
		$final_styles = [];
		if( is_array($decoded_saved_styles) ){
	...
	public function write_to_static_styles( $encoded_static_styles ) {
		...
		if ( $this->native_filesystem_setup( $file_access_credentials ) ) {
			...
			$file_path = path_join( NS\ULTIMATE_BLOCKS_PRO_DIR, $this->static_data_path );
